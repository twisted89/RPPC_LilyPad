...
void RefreshEnabledDevices(int updateDeviceList) {
	...
	for (int i = 0; i < dm->numDevices; i++) {
		...
		if ((dev->type == KEYBOARD && dev->api == IGNORE_KEYBOARD) ||
			...
			(dev->api == DS4 && config.gameApis.dualShock4) ||
			(dev->api == XINPUT && config.gameApis.xInput)))) {
			if (config.gameApis.dualShock4 && dev->api == DI && dev->displayName &&
...
wchar_t *GetCommandStringW(u8 command, int port, int slot) {
	...
	if (command >= 0x22 && command <= 0x29) {
		static wchar_t stick[2] = { 'L', 'R' };
		static wchar_t *dir[] = {
			L"Up", L"Right",
			L"Down", L"Left" };
		wsprintfW(temp, L"%c-Stick %s", stick[(command - 0x22) / 4], dir[(command - 0x22) & 3]);
		return temp;
	}
...
int BindCommand(Device *dev, unsigned int uid, unsigned int port, unsigned int slot, int command, int sensitivity, int turbo, int deadZone, int exponent);
...
void SelChanged(int port, int slot) {
	...
	if (!ffb) {
		...
		SetLogSliderVal(hWnd, IDC_SLIDER_exponent, GetDlgItem(hWnd, IDC_AXIS_EXPONENT), exponent);
...
void ChangeValue(int port, int slot, int *newSensitivity, int *newTurbo, int *newDeadZone, int *newExponent) {
...
int SaveSettings(wchar_t *file = 0) {
	...
	for (int port = 0; port < NumberOfPads; port++) {
		...
		for (int port = 0; port < NumberOfPads; port++) {
			for (int slot = 0; slot < 4; slot++) {
				for (int j = 0; j < dev->pads[port][slot].numBindings; j++) {
					...
					wsprintfW(temp2, L"0x%08X, %i, %i, %i, %i, %i, %i, %i", c->uid, port, b->command, b->sensitivity, b->turbo, slot, b->deadZone, b->Exponent);
...
int LoadSettings(int force, wchar_t *file) {
	...
	for (int port = 0; port < NumberOfPads; port++) {
		for (int slot = 0; slot < 4; slot++) {
			wchar_t temp[50];
			wsprintf(temp, L"Pad %i %i", port, slot);
			config.padConfigs[port][slot].type = (PadType)GetPrivateProfileInt(temp, L"Mode", Dualshock4Pad, file);
	...
	while (1) {
		...
		while (1) {
			...
			last = 1;
			unsigned int uid;
			int port, command, sensitivity, turbo, slot = 0, deadZone = 0, exponent = 0;
			int w = 0;
			char string[1000];
			while (temp2[w]) {
			...
			string[w] = 0;
			int len = sscanf(string, " %i , %i , %i , %i , %i , %i , %i , %i", &uid, &port, &command, &sensitivity, &turbo, &slot, &deadZone, &exponent);
			if (len >= 5 && type) {
				VirtualControl *c = dev->GetVirtualControl(uid);
				if (!c) c = dev->AddVirtualControl(uid, -1);
				if (c) {
					BindCommand(dev, uid, port, slot, command, sensitivity, turbo, deadZone, exponent);
				}
			}
		}
		j = 0;
...
INT_PTR CALLBACK DialogProc(HWND hWnd, unsigned int msg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_INITDIALOG:
	{
		...
		SetupLogSlider(GetDlgItem(hWnd, IDC_SLIDER_exponent));
		if (port || slot)
		...
		if (!selected || selected == 0xFF) {
			...
			if (dev) {
				...
				if (command == 0x7F && dev->api == IGNORE_KEYBOARD) {
					index = BindCommand(dev, uid, 0, 0, command, BASE_SENSITIVITY, 0, 0, 0);
				}
				else if (command < 0x30) {
					index = BindCommand(dev, uid, port, slot, command, BASE_SENSITIVITY, 0, 0, 0);
				}
				if (index >= 0) {
					PropSheet_Changed(hWndProp, hWnds[port][slot]);
					ListView_SetItemState(hWndList, index, LVIS_SELECTED, LVIS_SELECTED);
	...
	case WM_HSCROLL:
	{
		...
		else if (id == IDC_SLIDER_exponent) {
			...
			if (index >= 0) {
				...
				if (cbsel >= 0) {
					...
					if (GetBinding(port, slot, index, dev, b, ffb)) {
						...
						int index = BindCommand(dev, uid, port, slot, backup.command, backup.sensitivity, backup.turbo, backup.deadZone, backup.Exponent);
						ListView_SetItemState(hWndList, index, LVIS_SELECTED, LVIS_SELECTED);
				...
				if (i >= 0) {
					...
					if (index < (unsigned int)dm->numDevices) {
						...
						if (b) {
							...
							else if (dev->api == DS4 && dev->numFFAxes == 2) {
								needSet = 0;
								if (cmd == ID_BIG_MOTOR) {
									b->axes[0].force = BASE_SENSITIVITY;
								}
...
void UpdatePadList(HWND hWnd) {
	...
	int port;
	int index = 0;
	wchar_t *padTypes[] = { L"Unplugged", L"Dualshock 4" };
	for (port = 0; port < NumberOfPads; port++) {
		for (slot = 0; slot < 4; slot++) {
			...
			item.iSubItem = 1;
			if (2 < (unsigned int)config.padConfigs[port][slot].type) config.padConfigs[port][slot].type = Dualshock4Pad;
...
INT_PTR CALLBACK GeneralDialogProc(HWND hWnd, unsigned int msg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_INITDIALOG:
	{
		...
		if (SendMessage(hWndCombo, CB_GETCOUNT, 0, 0) == 0) {
			...
			SendMessage(hWndCombo, CB_ADDSTRING, 0, (LPARAM)L"Dualshock 4");
	...
	if (!DualShock4Possible()) {
		config.gameApis.dualShock4 = 0;
