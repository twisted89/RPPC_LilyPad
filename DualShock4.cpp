...
void TryInitDS4(usb_device *dev) {
	while (dev) {
...
void DS4Enum(unsigned int time) {
...
void DS4Check(unsigned int time) {
...
class DualShock4Device : public Device {
...
public:
	...
	DualShock4Device(int index, wchar_t *name, wchar_t *path) : Device(DS4, OTHER, name, path, L"DualShock 4") {
		writeCount = 0;
		writing = 0;
		writeQueued = 0;
	...
	for (int i=0; i<numDevs; i++) {
		if (foundDevs[i].caps.FeatureReportByteLength == 49 &&
			...
			foundDevs[i].caps.OutputReportByteLength == 49) {
				wchar_t temp[100];
				wsprintfW(temp, L"DualShock 4 #%i", index+1);
				dm->AddDevice(new DualShock4Device(index, temp, foundDevs[i].path));
				index++;
		}
